...
import { Text, Alert, Image, FlatList, TouchableHighlight, View, StyleSheet } from 'react-native';
...
class SavedEventsScreen extends React.Component {
    ...
    _setImageUriAndDisplay = event => {
        if (event.uri) {
            ...
            Alert.alert('No Saved Image!', 'We cannot display the original image since it hasn\'t been stored.');
        }
    };
    ...
    render() {
        ...
        return !this.state.displayImage && (this.state.imageUri !== null || this.state.imageUri !== undefined) ? (
            // check if there exist real events (if not display the dummy events from above)
            <View style={styles.container}>
                <FlatList
                    data={this.props.events.events.length === 0 ? this.state.data : this.props.events.events}
            ...
            <View style={styles.container}>
                <Text>The saved image for calender event: {`${this.state.imageId}`}</Text>

                <TouchableHighlight onPress={() => this._cancelDisplay()} style={styles.imageCancelButton}>
